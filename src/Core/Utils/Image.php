 ...
 *
 * <code>
 * $image = Image::fromFile('megadeath.jpg');
...
class Image
{
     ...
     * @param string $file
     * @param int|null $detectedFormat
     * @return static
     * @throws Exceptions\UnknownImageFileException if file not found or file type is not known
     */
	public static function fromFile(string $file, int &$detectedFormat = null)
	{
		if (!extension_loaded('gd')) {
			throw new Exceptions\NotSupportedException('PHP extension GD is not loaded.');
		}

		$detectedFormat = @getimagesize($file)[2]; // @ - files smaller than 12 bytes causes read error
		if (!isset(self::$formats[$detectedFormat])) {
			$detectedFormat = null;
			throw new Exceptions\UnknownImageFileException(is_file($file) ? "Unknown type of file '$file'." : "File '$file' not found.");
		}
		return new static(Callback::invokeSafe('imagecreatefrom' . image_type_to_extension($detectedFormat, false), [$file], function (string $message): void {
			throw new Exceptions\ImageException($message);
		}));
	}
	...
	public static function fromBlank(int $width, int $height, array $color = null)
	{
		...
		if ($width < 1 || $height < 1) {
			throw new Exceptions\InvalidArgumentException('Image width and height must be greater than zero.');
		}
	...
	protected function setImageResource($image)
	{
		if (!is_resource($image) || get_resource_type($image) !== 'gd') {
			throw new Exceptions\InvalidArgumentException('Image is not valid.');
		}
		$this->image = $image;
		return $this;
	}
	...
	public static function calculateSize(int $srcWidth, int $srcHeight, $newWidth, $newHeight, int $flags = self::FIT): array
	{
		...
		if ($flags & self::STRETCH) { // non-proportional
			if (empty($newWidth) || empty($newHeight)) {
				throw new Exceptions\InvalidArgumentException('For stretching must be both width and height specified.');
			}
			...
			if (empty($newWidth) && empty($newHeight)) {
				throw new Exceptions\InvalidArgumentException('At least width or height must be specified.');
			}
	...
	public function save(string $file, int $quality = null, int $type = null): void
	{
		if ($type === null) {
			...
			if (!isset($extensions[$ext])) {
				throw new Exceptions\InvalidArgumentException("Unsupported file extension '$ext'.");
			}
			$type = $extensions[$ext];
		}
	...
	public function send(int $type = self::JPEG, int $quality = null): void
	{
		if (!isset(self::$formats[$type])) {
			throw new Exceptions\InvalidArgumentException("Unsupported image type '$type'.");
		}
	...
	private function output(int $type, ?int $quality, string $file = null): void
	{
		...
		if (!$success) {
			throw new Exceptions\ImageException(error_get_last()['message']);
		}
	}
	...
	{
		throw new Exceptions\NotSupportedException('You cannot serialize or unserialize ' . self::class . ' instances.');
	}
}
